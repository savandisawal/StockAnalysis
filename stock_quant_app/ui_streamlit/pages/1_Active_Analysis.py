...
from ui_streamlit.components.charts import candlestick_chart, indicator_chart
...
if existing_models:
    try:
        pred = predict_next_day(ticker, include_fundamentals=False, include_macro=False)
        if pred:
...
tab_rsi, tab_macd, tab_bb, tab_adx, tab_vol = st.tabs(["RSI", "MACD", "Bollinger %B", "ADX", "Volume Z"])


def _info_box(text: str, color: str):
    st.markdown(f'<div style="background:{color}15;border:1px solid {color}40;border-radius:10px;padding:10px 16px;color:{color}">{text}</div>', unsafe_allow_html=True)
...
with tab_macd:
    ...
    _info_box(f'<b>MACD Histogram = {v:.3f}%</b> -- {"Bullish momentum" if v > 0 else "Bearish momentum"}', c)

with tab_bb:
    fig = indicator_chart(chart_df, "bb_pct_b", title="Bollinger %B", color="#FFD740")
    st.plotly_chart(fig, use_container_width=True)
    v = chart_df["bb_pct_b"].iloc[-1]
    _info_box(f'<b>%B = {v:.3f}</b> -- {"Near upper band (resistance)" if v > 0.8 else "Near lower band (support)" if v < 0.2 else "Mid-range"}', "#FFD740")
...
with tab_adx:
    ...
    _info_box(f'<b>ADX = {adx_v:.1f}</b> | Regime: <b>{regime_label.get(regime, "Unknown")}</b>', rc)
...
with st.expander("Macro Mood"):
    try:
        ...
        mood_color = COLORS["bullish"] if mood > 0.3 else COLORS["bearish"] if mood < -0.3 else COLORS["warning"]
        st.markdown(f'<div style="background:{mood_color}15;border:1px solid {mood_color}40;border-radius:12px;padding:16px;text-align:center;margin-bottom:16px"><div style="font-size:0.8rem;color:#9E9E9E;text-transform:uppercase;letter-spacing:1px">Macro Mood</div><div style="font-size:1.8rem;font-weight:800;color:{mood_color}">{mood_label}</div><div style="color:#78909C;font-size:0.85rem">Score: {mood:+.2f}</div></div>', unsafe_allow_html=True)
