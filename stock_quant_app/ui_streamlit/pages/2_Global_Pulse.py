...
from ui_streamlit.components.styles import (
    inject_global_css, styled_header, macro_indicator_card, section_header, COLORS,
)
...
if vix_snap and vix_snap.current_price:
    ...
    if vix_val > 25:
        vix_color, vix_label = COLORS["bearish"], "HIGH FEAR -- Elevated volatility, markets may be stressed"
    elif vix_val > 18:
    ...
    st.markdown(f'<div style="background:{vix_color}10;border-left:4px solid {vix_color};border-radius:0 10px 10px 0;padding:12px 18px;margin-top:16px;color:{vix_color};font-size:0.85rem">VIX at <b>{vix_val:.1f}</b> -- {vix_label}</div>', unsafe_allow_html=True)
...
with st.expander("Raw Data"):
    import pandas as pd
    data = [{
        "Indicator": s.name, "Ticker": s.ticker,
        "Current": s.current_price, "Prev Close": s.prev_close,
        "Change %": s.change_pct, "Date": s.fetch_date,
    } for s in snapshots]
