...
def styled_header(title: str, subtitle: str = "", icon: str = ""):
    ...
    sub_html = (
        f'<p style="color:#9E9E9E;font-size:0.95rem;margin-top:4px">{subtitle}</p>'
        if subtitle
        else ""
    )
...
def metric_card(label: str, value: str, delta: str = "", color: str = "#6C63FF", icon: str = ""):
    ...
    delta_html = (
        f'<span style="color:{delta_color};font-size:0.9rem;font-weight:600">{delta}</span>'
        if delta
        else ""
    )
...
def confidence_badge(confidence: float, direction: str):
    ...
    if direction == "Bullish":
        bg, border, dir_color, arrow = (
            "linear-gradient(135deg, #00E67620, #00E67605)",
            "#00E676",
            COLORS["bullish"],
            "^",
        )
        ...
        bg, border, dir_color, arrow = (
            "linear-gradient(135deg, #FF525220, #FF525205)",
            "#FF5252",
            COLORS["bearish"],
            "v",
        )
        ...
        bg, border, dir_color, arrow = (
            "linear-gradient(135deg, #78909C20, #78909C05)",
            "#78909C",
            COLORS["neutral"],
            "-",
        )
...
def macro_indicator_card(name: str, price: float | None, change: float | None, icon: str = ""):
    ...
    if change > 0:
        change_color, bg_accent, border_color = (
            COLORS["bullish"],
            "rgba(0, 230, 118, 0.06)",
            "rgba(0, 230, 118, 0.3)",
        )
